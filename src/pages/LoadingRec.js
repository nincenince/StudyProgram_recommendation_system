...
import {update_rec, destroy_recpay} from '../actions';
...
function LoadingRec(props) {
  ...
  useEffect(() => {
    setTimeout(async () => {
      //await fetch("http://127.0.0.1:8000/get/recommend/",
      await fetch("https://spr-system.herokuapp.com/get/recommend/",
      {
        method: 'post',
        headers: {
        ...
        .then(([responseData,response]) => {
          ...
          setTimeout(() => {
            setSuccess(true);
            dispatch(update_rec(responseData['info']));
